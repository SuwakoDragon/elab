...
return baseclass.extend({
	...
	renderMainMenu: function (tree, url, level) {
		...
		for (var i = 0; i < children.length; i++) {
			...
			var menuClass = hasChildren ? 'menu' : 'menu leaf';
			...
			// Close mobile sidebar after selecting a leaf item
			if (currentLevel === 1 && !hasChildren) {
				var linkNode = menuItem.querySelector('a');
				if (linkNode) {
					linkNode.addEventListener('click', L.bind(function () {
						...
						darkMask && darkMask.classList.remove('active');
						scrollbarArea && scrollbarArea.classList.remove('active');
					}, this));
				}
			}
